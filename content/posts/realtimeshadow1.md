...
自遮挡是因为shadowmap把场景的深度离散化成了若干个点，形成了类似与光源方向平行的阶梯状的分布，导致一个区域内深度都是一致的，但是深度是线性变化的，第二个pass并不能保证刚好采样到第一个pass选到的点，导致出现一个区域内的点深度比记录的深度远，使其被判断为被遮挡，而且光源越与表面平行，产生的每个阶梯的区域就越大，就有更多的部分被判断为被遮挡
